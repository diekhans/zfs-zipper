...
* initializing a zfs pool backup disk
     dd if=/dev/zero of=/dev/XX1 bs=1m count=128
     zpool create -O atime=off -O compression=lz4 -O dedup=on -m /media/zackup1 osprey_zackup1a disk0 disk1
...
* create mirrored pool
  zpool create -O atime=off -O compression=lz4 -O dedup=on -m /media/zackup1 osprey_zackup1a disk6 disk7
  zpool list
  zpool status
